...
<div class="container-fluid">
  <div class="row">
      ...
      <div class="offcanvas-md offcanvas-end bg-body-tertiary" tabindex="-1" id="sidebarMenu" aria-labelledby="sidebarMenuLabel">
        ...
        <div class="offcanvas-body d-md-flex flex-column p-0 pt-lg-3 overflow-y-auto">
          <ul class="nav flex-column">
			...
			<li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2 active" href="#" data-bs-toggle="modal" data-bs-target="#modalReporte">
                <svg class="bi"><use xlink:href="#file-earmark"/></svg>
                Reporte Mttos.
              </a>
            </li>
	...
	<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3">
		<h1 class="h2">Reporte de Registros de Mantenimiento</h1>
    </div>
  ...
  if (isset($_REQUEST['reporte']))
  {
	  ...
	  $sql_query = "select folio,fecha,tipo_equipo,serie,n_inventario,area,usuario_equipo,asignado from soportes where area = '$area' and fecha between '$fechai' and '$fechaf'";
	  $registros = mysqli_query($conecta,$sql_query);
	  
	  $query2 = "select folio,fecha,tipo_equipo,serie,n_inventario,area,usuario_equipo,asignado from soportes where fecha between '$fechai' and '$fechaf'";
	  $registros2 = mysqli_query($conecta,$query2);
	  
	  $total = mysqli_num_rows($registros);
	  $total2 = mysqli_num_rows($registros2);
	  
	  if ($area == 'Todas')
	  {
		  if ($total2 == 0)
		  {
			  echo '<tbody><tr><td colspan="5" class="text-center py-4"><div class="alert alert-info mb-0"><center>No existen registros con esa información.</center></div></td></tr></tbody>';
		  }
		  else
		  {
			  setlocale(LC_TIME, 'spanish');
			  
			  $fi = DateTime::createFromFormat('Y-m-d', $_GET['fi']);
			  $ff = DateTime::createFromFormat('Y-m-d', $_GET['ff']);
			  $me = strtotime($_GET['ff']);
			  
			  $diai = $fi->format('d');
			  $diaf = $ff->format('d');
			  $mes = strftime('%B',$me);
			  $año = $fi->format('Y'); // ambos son del mismo año
			  
			  echo '<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3">';
			  echo '<h2 class="h4">Registros del '.$diai.' al '.$diaf.' de '.$mes.' del '.$año.' de Todas las Áreas</h2></div>';
			  //Tabla de registros.
			  echo '<div class="table-responsive scroll">';
			  ...
			  echo '<th>Equipo</th>';
			  echo '<th>N. Serie</th>';
			  echo '<th>N. Inventario</th>';
			  echo '<th>Área</th>';
			  echo '<th>Asignado</th>';
			  echo '</tr>';
			  echo '</thead>';
			  echo '<tbody>';
			  while ($dato = mysqli_fetch_array($registros2))
			  {
				  echo '<tr>';
				  echo '<td>'.$dato['folio'].'</td>';
				  echo '<td>'.$dato['fecha'].'</td>';
				  echo '<td>'.$dato['tipo_equipo'].'</td>';
				  echo '<td>'.$dato['serie'].'</td>';
				  echo '<td>'.$dato['n_inventario'].'</td>';
				  echo '<td>'.$dato['area'].'</td>';
				  echo '<td>'.$dato['asignado'].'</td>';
				  echo '</tr>';
			  }
				  echo '</tbody>';
				  echo '</table>';
				  ...
				  echo '<div class="d-flex gap-3 justify-content-start">';
				  
				  echo '<form class="row m-0" action="reporte2.php" method="POST" autocomplete="off" target="_blank">';
				  echo '<input type="hidden" name="finicio"  class="form-control" value="'.$fechai.'">';
				  echo '<input type="hidden" name="ffin"  class="form-control" value="'.$fechaf.'">';
				  echo '<input type="hidden" name="di"  class="form-control" value="'.$diai.'">';
				  echo '<input type="hidden" name="df"  class="form-control" value="'.$diaf.'">';
				  echo '<input type="hidden" name="m"  class="form-control" value="'.$mes.'">';
				  echo '<input type="hidden" name="a"  class="form-control" value="'.$año.'">';
	  ...
	  else
	  {
		  ...
		  else
		  {
			  ...
			  echo '<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3">';
			  echo '<h2 class="h4">Registros del '.$diai.' al '.$diaf.' de '.$mes.' del '.$año.' del Área: '.$area.'</h2></div>';
			  //Tabla de registros.
			  echo '<div class="table-responsive scroll">';
			  echo '<table class="table table-hover table-striped">';
			  echo '<thead class="">';
			  ...
			  echo '<form class="row m-0" action="reporte.php" method="POST" autocomplete="off" target="_blank">';
			  echo '<input type="hidden" name="finicio"  class="form-control" value="'.$fechai.'">';
			  echo '<input type="hidden" name="ffin"  class="form-control" value="'.$fechaf.'">';
			  echo '<input type="hidden" name="ar"  class="form-control" value="'.$area.'">';
			  echo '<input type="hidden" name="di"  class="form-control" value="'.$diai.'">';
			  echo '<input type="hidden" name="df"  class="form-control" value="'.$diaf.'">';
			  echo '<input type="hidden" name="m"  class="form-control" value="'.$mes.'">';
			  echo '<input type="hidden" name="a"  class="form-control" value="'.$año.'">';
