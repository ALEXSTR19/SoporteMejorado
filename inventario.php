...
function datosArticuloValidos($datos) {
    return $datos['nombre'] !== '' && $datos['modelo'] !== '' && $datos['existencia'] >= 0 && $datos['minimo'] >= 0
...
if (!$error && $_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!hash_equals($_SESSION['inventario_csrf'], isset($_POST['csrf']) ? $_POST['csrf'] : '')) {
        ...
        if ($accion === 'articulo') {
            ...
            $res = $id ? mysqli_query($conecta, "SELECT foto FROM inventario_articulos WHERE id=$id AND activo=1") : false;
            ...
            if (!$error) {
                ...
                if ($ok) { if ($fotoNueva && $anterior['foto'] && is_file(__DIR__.'/'.$anterior['foto'])) unlink(__DIR__.'/'.$anterior['foto']); $mensaje = 'Articulo actualizado correctamente.'; }
                else { if ($fotoNueva && is_file(__DIR__.'/'.$fotoNueva)) unlink(__DIR__.'/'.$fotoNueva); $error = 'No se pudo actualizar el articulo.'; }
            }
