...
$filtro_asignado = "AND asignado = '$nombre_usuario'";
...
if ($action === 'detalles') {
    ...
    $q_detalles = "SELECT id, folio, fecha, area, asignado, falla, estado FROM soportes WHERE fecha BETWEEN '$fecha_inicio' AND '$fecha_fin' $filtro_asignado $where_estado ORDER BY id DESC LIMIT 500";
...
$q = mysqli_query($conecta, "SELECT COUNT(*) as total FROM soportes WHERE fecha BETWEEN '$fecha_inicio' AND '$fecha_fin' $filtro_asignado");
$r = mysqli_fetch_assoc($q);
$data['total_soportes'] = (int)$r['total'];

// Por estado
$q = mysqli_query($conecta, "SELECT estado, COUNT(*) as total FROM soportes WHERE fecha BETWEEN '$fecha_inicio' AND '$fecha_fin' $filtro_asignado GROUP BY estado");
...
$q = mysqli_query($conecta, "SELECT COUNT(*) as total FROM soportes WHERE fecha BETWEEN '$periodo_anterior_ini' AND '$periodo_anterior_fin' $filtro_asignado");
$r = mysqli_fetch_assoc($q);
...
$q = mysqli_query($conecta, "
    ...
    FROM soportes 
    WHERE fecha BETWEEN '$fecha_inicio' AND '$fecha_fin' $filtro_asignado
    AND asignado != ''
    GROUP BY asignado
    ORDER BY total DESC
");
