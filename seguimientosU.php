...
<!doctype html>
    ...
    <style>
	  ...
	  td.col-diag {  /* columna Diagnóstico */
      width: 100%;
      }
	  td:last-child {
      width: 120px;
...
<?php if ($esMaster): ?>
            ...
            <li class="nav-item"><a class="nav-link d-flex align-items-center gap-2 active" href="seguimientosU.php"><svg class="bi"><use xlink:href="#clipboard-check"/></svg>Reporte Seguim.</a></li>
<?php else: ?>
            <li class="nav-item">
			...
			<li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2 active" href="seguimientosU.php">
...
<?php endif; ?>
	...
	<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3">
		...
		<?php 
		if ($esMaster) {
			echo "Listado General de Seguimientos";
		} else {
			echo "Listado de Seguimientos de " . $_SESSION['nombreuser']; 
		}
		?>
		</h1>
    </div>
	...
	<?php if ($esMaster): 
		$filtro_usuario = isset($_GET['usuario_filtro']) ? mysqli_real_escape_string($conecta, $_GET['usuario_filtro']) : '';
		$qUsuarios = mysqli_query($conecta, "SELECT DISTINCT asignado FROM soportes WHERE asignado IS NOT NULL AND asignado != '' ORDER BY asignado ASC");
	?>
	<div class="mb-3">
		<form method="GET" action="seguimientosU.php" class="d-flex align-items-center">
			<label for="usuario_filtro" class="form-label me-2 mb-0 fw-bold">Filtrar por Usuario:</label>
			<select name="usuario_filtro" id="usuario_filtro" class="form-select w-auto me-2" onchange="this.form.submit()">
				<option value="">Todos los usuarios</option>
				<?php while($u = mysqli_fetch_assoc($qUsuarios)): ?>
					<option value="<?php echo htmlspecialchars($u['asignado']); ?>" <?php echo ($filtro_usuario === $u['asignado']) ? 'selected' : ''; ?>>
						<?php echo htmlspecialchars($u['asignado']); ?>
					</option>
				<?php endwhile; ?>
			</select>
		</form>
	...
	<?php
	// Master ve TODOS los reportes de seguimiento, o filtra por usuario. Usuario normal solo los suyos.
	if ($esMaster) {
		if ($filtro_usuario !== '') {
			$query = "SELECT s.folio, MAX(e.nota) AS nota, s.asignado 
                      FROM soportes s 
                      LEFT JOIN evidencias e ON s.folio = e.folio 
                      LEFT JOIN fotos f ON s.folio = f.folio 
                      WHERE s.asignado = '$filtro_usuario' AND (e.folio IS NOT NULL OR f.folio IS NOT NULL) 
                      GROUP BY s.folio, s.asignado";
		} else {
			$query = "SELECT s.folio, MAX(e.nota) AS nota, s.asignado 
                      FROM soportes s 
                      LEFT JOIN evidencias e ON s.folio = e.folio 
                      LEFT JOIN fotos f ON s.folio = f.folio 
                      WHERE (e.folio IS NOT NULL OR f.folio IS NOT NULL) 
                      GROUP BY s.folio, s.asignado";
		}
	} else {
		$query = "SELECT s.folio, MAX(e.nota) AS nota, s.asignado 
                  ...
                  LEFT JOIN evidencias e ON s.folio = e.folio 
                  LEFT JOIN fotos f ON s.folio = f.folio 
                  WHERE s.asignado = '$_SESSION[nombreuser]' AND (e.folio IS NOT NULL OR f.folio IS NOT NULL) 
                  GROUP BY s.folio";
	}
...
<div class="table-responsive scroll">
    ...
    <tr class="letra">
		<th>Folio</th>
		<?php if ($esMaster): ?><th>Usuario</th><?php endif; ?>
		<th>Diagnóstico</th>
	...
	{ ?>
    <tr data-folio="<?php echo htmlspecialchars($dato['folio']); ?>">
		<td><?php echo htmlspecialchars($dato['folio']); ?></td>
		<?php if ($esMaster): ?><td><?php echo htmlspecialchars($dato['asignado']); ?></td><?php endif; ?>
		<td class="col-diag truncate-text" title="<?php echo htmlspecialchars($dato['nota']); ?>"><?php echo htmlspecialchars($dato['nota']); ?></td>
        <td>
        <center><a href="diagnostico.php?ref=<?php echo htmlspecialchars($dato['folio']); ?>" target="_blank">
		<button type="submit" class="btn btn-can">
