...
<section class="card panel"><div class="card-header bg-white py-3"><h2 class="h5 mb-0">Historial completo de movimientos</h2></div><div class="table-responsive"><table class="table table-striped table-hover mb-0"><thead><tr><th>Fecha y hora</th><th>Movimiento</th><th>Referencia</th><th>Entrada</th><th>Salida</th><th>Saldo</th><th>Seguimiento / responsable</th><th>Registrado por</th></tr></thead><tbody>
<?php foreach ($movimientos as $m): $entrada=(float)$m['cantidad']>0?(float)$m['cantidad']:0; $salida=(float)$m['cantidad']<0?abs((float)$m['cantidad']):0; ?><tr><td class="text-nowrap"><?php echo e(date('d/m/Y H:i', strtotime($m['creado_en']))); ?></td><td><span class="badge bg-<?php echo $salida?'danger':($m['tipo']==='Ajuste'?'warning text-dark':'success'); ?>"><?php echo e($m['tipo']); ?></span></td><td><?php echo e($m['referencia'] ?: 'Sin referencia'); ?></td><td class="text-success fw-bold"><?php echo $entrada?'+'.cantidadKardex($entrada):'—'; ?></td><td class="text-danger fw-bold"><?php echo $salida?'-'.cantidadKardex($salida):'—'; ?></td><td class="fw-bold"><?php echo cantidadKardex($m['saldo']); ?></td><td class="detail"><?php if($m['destino']): ?><strong><?php echo e($m['destino']); ?></strong> · <?php echo e($m['motivo']); ?><br><small>Responsable: <?php echo e($m['responsable']); ?> · Estado: <?php echo e($m['estado_salida']); ?><?php if($m['observaciones_salida']): ?><br>Observaciones: <?php echo e($m['observaciones_salida']); ?><?php endif; ?></small><?php else: ?><span class="text-secondary"><?php echo $m['tipo']==='Alta'?'Alta inicial del articulo':'Movimiento interno de inventario'; ?></span><?php endif; ?></td><td><?php echo e($m['usuario']); ?></td></tr><?php endforeach; ?>
