...
$edt=$_GET['ref'];
		$consulta = "SELECT * FROM soportes WHERE id = $edt";
		$resultado = mysqli_query($conecta,$consulta);
		while ($dato=mysqli_fetch_array($resultado)) {
			$folio = $dato['folio'];
			$fecha = $dato['fecha'];
			$hora = $dato['hora'];
			$equipo = $dato['tipo_equipo'];
			$marca = $dato['marca'];
			$modelo = $dato['modelo'];
			$serie = $dato['serie'];
			$inventario = $dato['n_inventario'];
			$a = $dato['area'];
			$mantenimiento = $dato['tipo_mantto'];
			$usuario = $dato['usuario_equipo'];
			$reporta = $dato['usuario_reporte'];
			$contacto = $dato['contacto'];
			$falla = $dato['falla'];
			$asigna = $dato['asignado'];
			$user = $dato['usuario'];
         }
				
/*if (isset($_REQUEST['guardar']))
{ 
    $sql_query = "update soportes set tipo_equipo = '$_POST[tipo_equipo]',marca = '$_POST[marca]',modelo = '$_POST[modelo]',serie = '$_POST[serie]',n_inventario = '$_POST[n_inventario]',area = '$_POST[area]',tipo_mantto = '$_POST[tipo_mantto]',usuario_equipo = '$_POST[usuario_equipo]',usuario_reporte = '$_POST[usuario_reporte]',contacto = '$_POST[movil]',falla = '$_POST[falla]',asignado = '$_POST[usuarioa]' where id = '$GLOBALS[clave]'";
...
<!doctype html>
   ...
   .evidencia-item .btn-eliminar-evi {
    ...
    justify-content: center; padding: 0; transition: background 0.2s;
   }
   .evidencia-item .btn-eliminar-evi:hover {
    background: rgba(220,53,69,1);
   }
   .nota-item {
    background: #f1f3f5; border-radius: 6px; padding: 10px 14px;
    margin-bottom: 8px; border-left: 3px solid #882e41;
   }
   .lightbox-overlay {
   ...
   .drop-zone {
    border: 2px dashed #adb5bd; border-radius: 8px; padding: 25px;
    text-align: center; cursor: pointer; transition: all 0.3s;
    background: #fff; color: #6c757d;
   }
   .drop-zone:hover, .drop-zone.dragover {
    border-color: #882e41; background: #fdf0f3; color: #882e41;
   }
   ...
   .preview-item {
    ...
    border: 1px solid #dee2e6; aspect-ratio: 1;
   }
...
<div class="container-fluid">
  <div class="row">
      ...
      <div class="offcanvas-md offcanvas-end bg-body-tertiary" tabindex="-1" id="sidebarMenu" aria-labelledby="sidebarMenuLabel">
        ...
        <div class="offcanvas-body d-md-flex flex-column p-0 pt-lg-3 overflow-y-auto">
          <ul class="nav flex-column">
			...
			<li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" href="altasU.php">
                <svg class="bi"><use xlink:href="#people"/></svg>
                Registro usuarios
              </a>
            </li>
	...
	<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3">
		<h1 class="h2">Editor de Registros del Departamento de TI</h1>
    </div>
	
 <form class="row g-3" action="actualizar.php" method="POST" autocomplete="off">
    	  <?php
    	       	if (isset ($msg))
	            {echo "<h4 align='center'>$msg con el folio: $folio</h4>";}
			    if (isset ($msg2))
  ...
  <div class="col-md-2">
    ...
    <input type="date" value="<?php echo $fecha;?>" class="form-control" name="fecha" disabled>
    <input type="hidden" value="<?php echo $fecha;?>" class="form-control" name="fechareg">
  </div>
   <div class="col-md-2">
    <label class="form-label">Hora de Registro</label>
    <input type="time" value="<?php echo $hora;?>" class="form-control" name="hora" disabled>
  </div>  
    <div class="col-md-6">
    <label class="form-label">Nombre de Usuario</label>
    <input type="text" value="<?php echo $user;?>" class="form-control" name="usuario" disabled>
  </div> 
  <?php
  ...
  <div class="col-md-3">
     <label class="form-label">Tipo de Equipo</label>
       <select class="form-control select" name="tipo_equipo" required>
		 <?php 
		 while($row = mysqli_fetch_array($result))
			{
				$selected = ($row[1] == $equipo) ? 'selected' : '';
 ...
 <?php
  ...
  <div class="col-md-3">
     <label class="form-label">Marca</label>
       <select class="form-control select" name="marca" required>
		 <?php 
		 while($mar = mysqli_fetch_array($resulta))
			{
				$selected = ($mar[1] == $marca) ? 'selected' : '';
			echo"<option value='{$mar[1]}' $selected>{$mar[1]}</option>";
			}
			?>
	   </select>
  </div>
   <div class="col-md-2">
    ...
    <input type="text" value="<?php echo $modelo;?>" class="form-control" name="modelo" required>
  </div>
  <div class="col-md-2">
    <label class="form-label">Serie</label>
    <input type="text" value="<?php echo $serie;?>" class="form-control" name="serie" >
  </div>
   <div class="col-md-2">
    <label class="form-label">No. Inventario</label>
    <input type="text" value="<?php echo $inventario;?>" class="form-control" name="n_inventario" >
  </div>
  <?php
	$seleccion = "";
	if($_SERVER['REQUEST_METHOD']=='POST')
  ...
  <div class="col-md-2">
     <label class="form-label">Área</label>
       <select class="form-control select" name="area" required>
		 <?php 
		 while($area = mysqli_fetch_array($resultado))
			{
				$selected = ($area[1] == $a) ? 'selected' : '';
			echo"<option value='{$area[1]}' $selected>{$area[1]}</option>";
			}
   ...
   <div class="col-md-2">
     <label class="form-label">Tipo Mantto.</label>
       <select class="form-control select" name="tipo_mantto" required>
		 <option value="Preventivo" <?php if ($mantenimiento == 'Preventivo') echo 'selected'; ?>>Preventivo</option>
         <option value="Correctivo" <?php if ($mantenimiento == 'Correctivo') echo 'selected'; ?>>Correctivo</option>
         <option value="Perfectivo" <?php if ($mantenimiento == 'Perfectivo') echo 'selected'; ?>>Perfectivo</option>
		 <option value="Soporte Técnico" <?php if ($mantenimiento == 'Soporte Técnico') echo 'selected'; ?>>Soporte Técnico</option>
	   </select>
  </div>
   <div class="col-md-3">
    <label class="form-label">Usuario de Equipo</label>
    <input type="text" value="<?php echo $usuario;?>" class="form-control" name="usuario_equipo" required>
  </div>
   <div class="col-md-3">
    <label class="form-label">Quien reporta</label>
    <input type="text" value="<?php echo $reporta;?>" class="form-control" name="usuario_reporte" required>
  </div>
  <div class="col-md-2">
    <label class="form-label">Móvil de Contacto</label>
    <input type="text" value="<?php echo $contacto;?>" class="form-control" name="movil" required>
    </div>
  <div class="col-md-10">
    <label class="form-label">Descripción de la falla</label>
  <textarea name="falla" class="form-control"  rows="4" required><?php echo $falla;?></textarea>
  <input type="hidden" name="token"  class="form-control" value="<?php echo $token;?>">
  <input type="hidden" name="edit" class="form-control" value="<?php echo $edt; ?>">
  </div>
      <?php
	$sele = "";
	if($_SERVER['REQUEST_METHOD']=='POST')
     ...
     <label class="form-label">Se asigna a:</label>
       <select class="form-control select" name="usuarioa" required>
		 ...
		 while($usera = mysqli_fetch_array($resultau))
			{
				$selected = ($usera[1] == $asigna) ? 'selected' : '';
			echo"<option value='{$usera[1]}' $selected>{$usera[1]}</option>";
			}
...
<!-- Sección de Notas y Evidencias -->
<div class="seccion-evidencias">
<h5><i class="bi bi-journal-text"></i> Notas / Diagnóstico del Folio: <strong><?php echo htmlspecialchars($folio); ?></strong></h5>
<?php
$sql_notas = "SELECT * FROM evidencias WHERE folio = '" . mysqli_real_escape_string($conecta, $folio) . "' ORDER BY id ASC";
$res_notas = mysqli_query($conecta, $sql_notas);
if ($res_notas && mysqli_num_rows($res_notas) > 0) {
    while ($nota = mysqli_fetch_assoc($res_notas)) {
        echo '<div class="nota-item">' . nl2br(htmlspecialchars($nota['nota'])) . '</div>';
    }
} else {
    echo '<p class="text-muted"><i class="bi bi-inbox"></i> No hay notas registradas para este folio.</p>';
}
?>

<hr>
<h5><i class="bi bi-images"></i> Evidencias Fotográficas</h5>
...
<div id="sinEvidencias" class="text-center text-muted small py-2 d-none">
  <i class="bi bi-camera"></i> No hay evidencias fotográficas para este folio.
</div>

<hr>
<h6><i class="bi bi-cloud-arrow-up"></i> Subir nuevas evidencias</h6>
...
<button type="button" class="btn btn-sm mt-2" style="background-color:#882e41;color:#fff;" id="btnSubirArchivos" disabled>
...
<script>
var folioActual = '<?php echo htmlspecialchars($folio); ?>';
...
function cargarEvidencias() {
    ...
    fetch('gestionar_evidencias.php?accion=listar&folio=' + encodeURIComponent(folioActual))
        ...
        if (data.success && data.fotos.length > 0) {
            data.fotos.forEach(function(foto) {
                ...
                div.innerHTML = '<img src="' + foto.foto + '" alt="Evidencia" onclick="verImagen(\'' + foto.foto + '\')">' +
                    '<button type="button" class="btn-eliminar-evi" onclick="eliminarEvidencia(' + foto.id + ')" title="Eliminar"><i class="bi bi-x"></i></button>';
...
function eliminarEvidencia(id) {
    ...
    .then(function(r){ return r.json(); })
    .then(function(data) {
        if (data.success) { cargarEvidencias(); }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    inputFile.addEventListener('change', function() {
        agregarArchivos(this.files); this.value = '';
    });

    function agregarArchivos(files) {
        for (var i = 0; i < files.length; i++) {
            if (archivosParaSubir.length >= 5) { alert('Máximo 5 archivos'); break; }
            if (files[i].size > 15 * 1024 * 1024) { alert('El archivo ' + files[i].name + ' excede el límite de 15MB permitidos.'); continue; }
    ...
    function actualizarPreview() {
        ...
        archivosParaSubir.forEach(function(file, index) {
            ...
            btn.type = 'button'; btn.className = 'btn-quitar';
