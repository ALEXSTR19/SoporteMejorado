...
<div class="container-fluid">
  <div class="row">
      ...
      <div class="offcanvas-md offcanvas-end bg-body-tertiary" tabindex="-1" id="sidebarMenu" aria-labelledby="sidebarMenuLabel">
        ...
        <div class="offcanvas-body d-md-flex flex-column p-0 pt-lg-3 overflow-y-auto">
          <ul class="nav flex-column">
			...
			<li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2 active" href="#" data-bs-toggle="modal" data-bs-target="#modalKardex">
	...
	<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3">
		<h1 class="h2">Kardex del Equipo con el Numero de Inventario: <?php echo $_GET['ni']; ?></h1>
    </div>
<?php
if (isset($_GET['ni']))
{
	...
	$sql_query = "SELECT s.id, s.folio, s.fecha, s.tipo_equipo, s.falla, e.nota FROM soportes s LEFT JOIN ( SELECT *, ROW_NUMBER() OVER (PARTITION BY folio ORDER BY id DESC) AS rn FROM evidencias ) e ON s.folio = e.folio AND e.rn = 1 WHERE s.n_inventario = '$inventario' ORDER BY s.fecha DESC";
...
<div class="table-responsive scroll">
	<table class="table table-hover table-striped">
        ...
        <?php while ($dato = mysqli_fetch_array($registros)){?>
			<tr>
				<td class="col-fecha"><?php echo htmlspecialchars($dato['fecha'] ?? ''); ?></td>
				<td class="col-equipo"><?php echo htmlspecialchars($dato['tipo_equipo'] ?? ''); ?></td>
				<td onclick="window.open('registro.php?ref=<?php echo urlencode($dato['id']); ?>', '_blank')" style="cursor: pointer;"><?php echo htmlspecialchars($dato['falla'] ?? ''); ?></td>
				<td onclick="window.open('diagnostico.php?ref=<?php echo urlencode($dato['folio']); ?>', '_blank')" style="cursor: pointer;"><?php echo htmlspecialchars($dato['nota'] ?? ''); ?></td>
			</tr>
        <?php }?>
		<?php }?>
		</tbody>
	...
	<form class="row m-0" action="pdfkardex.php" method="POST" autocomplete="off" target="_blank">
	<input type="hidden" name="in"  class="form-control" value="<?php echo $inventario; ?>">
	<input type="hidden" name="equipo"  class="form-control" value="<?php echo htmlspecialchars($dato['tipo_equipo']); ?>">
	<button type="submit" class="btn btn-primary" >Imprimir</button>
	</form>
	</div>
</div>
<?php
}//Final del if del botón buscar.
?>
    </main>
